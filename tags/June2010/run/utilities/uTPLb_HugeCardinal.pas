...
{$ENDIF}
    ...
    constructor CreateZero( MaxBits1: integer; Pool1: TMemoryStreamPool);
    constructor CreateRandom( Bits1, MaxBits1: integer; ExactBitLength: boolean; Pool1: TMemoryStreamPool);
    ...
    constructor CreateSmall( Value: uint64; MaxBits1: integer; Pool1: TMemoryStreamPool);
    constructor CreateAsClone( Master: THugeCardinal; Pool1: TMemoryStreamPool);
    constructor CreateAsSizedClone( MaxBits1: integer; Master: THugeCardinal; Pool1: TMemoryStreamPool);
    constructor CreateFromStreamIn(
                       MaxBits1: integer; ByteOrder:
                       TByteOrder; Stream: TStream; Pool1: TMemoryStreamPool);
...
uses SysUtils, uTPLb_StreamUtils, Math, uTPlb_Random,
...
constructor THugeCardinal.CreateAsClone( Master: THugeCardinal; Pool1: TMemoryStreamPool);
begin
FPool := Pool1;
...
constructor THugeCardinal.CreateAsSizedClone( MaxBits1: integer; Master: THugeCardinal; Pool1: TMemoryStreamPool);
var
  DataByteLen: integer;
begin
FPool := Pool1;
...
if Master.BitLength > FMaxBits then
  raise Exception.Create( 'THugeCardinal.CreateAsSizedClone overflow.');
...
constructor THugeCardinal.CreateFromStreamIn(
  MaxBits1: integer; ByteOrder: TByteOrder; Stream: TStream; Pool1: TMemoryStreamPool);
begin
FPool := Pool1;
FMaxBits := ComputedNeededSize( MaxBits1);
FValue   := NewMemoryStream( FMaxBits);
...
constructor THugeCardinal.CreateSmall( Value: uint64; MaxBits1: integer; Pool1: TMemoryStreamPool);
begin
CreateZero( MaxBits1, Pool1);
if Value > 0 then
  begin
...
constructor THugeCardinal.CreateZero( MaxBits1: integer; Pool1: TMemoryStreamPool);
begin
FPool := Pool1;
FMaxBits := ComputedNeededSize( MaxBits1);
FBits := 0;
...
var
  ...
  begin
  FBits := -1;
  raise Exception.Create( 'THugeCardinal.Add overflow.')
  end;
...
if SourceBytes > FValue.Size then
  raise Exception.Create( 'Cannot assign THugeCardinal because source is too big' +
...
var
  ...
  begin
  FBits := -1;
  raise Exception.Create( 'THugeCardinal.Increment overflow.')
  end;
...
while ((Compare in [rGreaterThan, rEqualTo]) or (ShiftAmnt > 0)) and
  ...
  if Compare in [rGreaterThan, rEqualTo] then
    ...
    if RequiresQuotient and (ShiftAmnt > Quotient.MaxBits) then
      raise Exception.Create('THugeCardinal.Divide overflow.');
    if RequiresQuotient then
...
var
  ...
  begin
  FBits := -1;
  raise Exception.Create( 'MulPower2 overflow.')
  end;
...
var
  ...
  begin
  FBits := -1;
  raise Exception.Create( 'MulSmall overflow.')
  end;
...
if BitLength > NewMaxBits then
  raise Exception.Create( 'THugeCardinal.Resize overflow');
...
if isZero then
  begin
  Assert( Exponent <> 0, 'Zero raised to zero is an invalid operation.');
  exit
  end;
if Exponent = 0 then
  begin
...
if (RequiredBits < Exponent) or (RequiredBits > MaxBits) then
  raise Exception.Create('Power overflow');
...
var
  ...
  begin
  FBits := -1;
  raise Exception.Create( 'THugeCardinal.Subtract overflow.')
  end;
...
if SizeToOutput < ((BitLength + 7) div 8) then
  raise Exception.Create('THugeCardinal.StreamOut overflow.');
ZeroByte  := 0;
...
var
  ...
  if not Ok then
    raise Exception.Create('THugeCardinal.StreamIn overflow.')
  end;
