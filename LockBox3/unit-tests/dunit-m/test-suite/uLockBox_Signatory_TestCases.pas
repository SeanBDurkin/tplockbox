...
interface
uses DUnitM.UnitTestFramework, TPLB3.MemoryStreamPool, Classes, TPLB3.Signatory,
...
TRSASignatory_TestCase = class( TTestCase)
  ...
  published
    [Test] procedure Signatory_InversionTests_Version0Keys;
    [Test] procedure Signatory_InversionTests_Version1Keys;
  end;
...
uses SysUtils, TPLB3.StreamUtils, TPLB3.Constants, TPLB3.Asymetric;
...
Base64_to_stream( Key1_v0, FKey);
Base64_to_stream( Key2_v1, FKey_v1);
FKey.Position := 0;
FKey_v1.Position := 0;
...
Assert.IsTrue( FSig.Sign( FDocument, FSignature), 'RSA signature generation failed.');
DisplayStream( FSignature);
Assert.IsTrue( FSig.Verify( FDocument, FSignature)<>vFail, 'RSA signature verification failed.')
end;
