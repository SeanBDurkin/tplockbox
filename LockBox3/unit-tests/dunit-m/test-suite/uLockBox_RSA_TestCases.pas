...
TRSA_TestCase = class( TTestCase)
  ...
  published
    [Test] procedure ChineseRemainderTheorem;
    [Test] procedure Test_Compute_RSA_Fundamentals;
    [IgnoreMemoryLeaks]
    [Test] procedure Test_RSA_Primitives;
    [Test] procedure Cardinal2Stream_InversionTests;
    [Test] procedure RSAES_OAEP_ENCDECRYPT;
  end;


[TestFixture('RSA.Codec')]
...
uses SysUtils, TPLB3.HashDsc, TPLB3.BinaryUtils, TPLB3.StreamUtils,
     ...
     TPLB3.IntegerUtils, TPLB3.RSA_Primitives, Forms;
...
const
  Test_n: utf8string = // n (little-endien; base64)
  ...
  Test_e: utf8string = // e (little-endien; base64)
    'AQAB';
  Test_e_bits = 17;

  Test_d: utf8string = // d (little-endien; base64)
  ...
  Test_AES256Key: utf8string = // (base64; LoadFromStream)
...
Assert.IsTrue( i1.Compare( i2) = rEqualTo, 'Cardinal --> Stream inversion test.');
...
Assert.IsTrue( CompareMemoryStreams( s1, s2), 'Stream --> Cardinal inversion test.');
...
for j := 1 to 3 do
  ...
  Assert.IsTrue( Validate_RSA_Fundamentals( n, e, d, Totient), 'Failed to generate RSA key pair.');
  for i := 1 to 10 do
    begin
    ...
    Assert.IsTrue( RSAES_OAEP_ENCRYPT( n, e, Plaintext, Ciphertext), 'RSAES_OAEP_ENCRYPT');
    Ciphertext.Position := 0;
    Recon := TMemoryStream.Create;
    Assert.IsTrue( RSAES_OAEP_DECRYPT( d, n, Ciphertext, Recon, p, q, dp, dq, qinv), 'RSAES_OAEP_DECRYPT');
    Assert.IsTrue( CompareMemoryStreams( Plaintext, Recon), 'RSAES_OAEP_ENCRYPT/RSAES_OAEP_DECRYPT inversion.');
    Plaintext.Free;
    Ciphertext.Free;
    recon.Free;
    Application.ProcessMessages;
...
if not FwasAborted then
  Assert.IsTrue( Validate_RSA_Fundamentals( n, e, d, Totient),
...
begin
  ...
  Assert.IsTrue( m1.Compare( m2) = rEqualTo, Msg);
  m1.Free;
  m2.Free
end;
...
var
  ...
  function MakeHuge( const Definition: utf8string; BitLen: cardinal)
    : THugeCardinal;
  begin
  Tmp.Size := 0;
...
try
Assert.IsTrue( PlainText.Size <= RSAES_OAEP_ENCRYPT_MaxByteLen( n),
  Format('Plaintext too big (%d) for RSA keys.',[PlainText.Size]));

Assert.IsTrue( RSAES_OAEP_ENCRYPT( n, e, Plaintext, Ciphertext),
  'RSAES_OAEP_ENCRYPT');

Assert.IsTrue( RSAES_OAEP_DECRYPT( d, n, Ciphertext, Recon, p, q, dp, dq, qinv),
  'RSAES_OAEP_DECRYPT');

Assert.IsTrue( CompareMemoryStreams( Plaintext, Recon),
...
Assert.Warn( 'TRSA_TestCase.Test_RSA_Primitives() test has known memory leaks.');
end;
...
const
 ...
 RSA_1024_SampleKey1: utf8string =
 ...
 RSA_1024_SampleKey2: utf8string =
...
case Codec.AsymetricKeySizeInBits of
  ...
  else  Assert.IsTrue( False, 'No test case yet for this key size.');
  end;
...
Assert.AreEqual( x, recon_x, 'RSA Crypto');
end;
