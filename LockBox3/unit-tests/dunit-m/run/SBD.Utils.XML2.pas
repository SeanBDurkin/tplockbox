...
unit SBD.Utils.XML2;
interface
uses XMLIntf, Classes, FMTBcd, Generics.Defaults;
...
{$REGION 'Internal'}
{$if CompilerVersion >= 22.0}
type TTokenizer = class( TInterfacedObject, IEnumerable<RMatchFragment>)
  private
    sHaystack: string;
...
{$ifdef MSWINDOWS}
const
  DefaultNamespaceBase = 'http://www.seanbdurkin.id.au/pascaliburnus2/stylesheet';
{$endif}
...
uses XMLDoc, xmldom, SysUtils, StrUtils, Math, Character, Generics.Collections
     {$ifdef MSWINDOWS}
       , ShlObj, Windows, msxmldom, msxml
     {$endif}
     , Contnrs, IOUtils, EncdDecd;


{$ifdef MSWINDOWS}
type IXMLDOMDocument2 = interface(IXMLDOMDocument)
...
type TXMLDocProc = reference to procedure( const poDoc: IXMLDocument);
...
// Replacement function for SysUtils.EncodeDate()
begin
...
var
  ...
  begin
    result := SBD.Utils.XML2.ReadWord( psEncodedValue, StartPos, Length, MinVal, MaxVal, Val1);
  end;
...
var
  ...
  function ReadWord( StartPos, Length: integer; MinVal, MaxVal: word; var Val1: word): boolean;
  begin
    result := SBD.Utils.XML2.ReadWord( sEncoded, StartPos, Length, MinVal, MaxVal, Val1);
  end;
