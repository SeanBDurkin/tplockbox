...
TSimpleCodec = class( TInterfacedPersistent, ICodec, IBlockCipherSelector,
    IEventOrigin, ICodec_WithAsymetricSupport)
  private
    FMode: TCodecMode;
...
FisUserAborted := False;
    ...
    try
      ...
      OutputSeedSize := FParameterizedStreamCipher.SeedByteSize;
      if OutputSeedSize = -1 then
          ...
          begin
          if isVariableSeedSize( FParameterizedStreamCipher, MinSize, MaxSize) then
            begin
            if InputSeedSize > MaxSize then
                OutputSeedSize := MaxSize
              else if InputSeedSize < MinSize then
                OutputSeedSize := MinSize
            end;
