...
uses Classes, uTPLb_BaseNonVisualComponent, uTPLb_StreamCipher,
     uTPLb_BlockCipher, contnrs, uTPLb_HashDsc;
...
uses
  ...
  // Hashes
  uTPLb_SHA1, uTPLb_MD5,

  // Stream mode
  uTPLb_StreamToBlock, uTPLb_Base64, uTPLb_RSA_Engine,
...
for j := 0 to FChildLibraries.Count - 1 do
  begin
  result := (FChildLibraries[j] as TCryptographicLibrary)
...
var
  ...
  for j := 0 to FChildLibraries.Count - 1 do
    FindStreamProgIds( FChildLibraries[j] as TCryptographicLibrary)
  end;
  ...
  for j := 0 to FChildLibraries.Count - 1 do
    FindBlockProgIds( FChildLibraries[j] as TCryptographicLibrary)
  end;
...
var
  ...
  for j := 0 to FChildLibraries.Count - 1 do
    FindChainProgIds( FChildLibraries[j] as TCryptographicLibrary)
  end;
begin
result := nil;
if FisDestroying then exit;
...
var
  ...
  for j := 0 to Lib.FHashes.Count - 1 do
    if Supports( Lib.FHashes[j], IBlockChainingModel, Hash) and
  ...
  for j := 0 to FChildLibraries.Count - 1 do
    FindHashProgIds( FChildLibraries[j] as TCryptographicLibrary)
  end;
begin
result := nil;
if FisDestroying then exit;
...
for j := 0 to FChildLibraries.Count - 1 do
  begin
  W := FChildLibraries[j] as TCryptographicLibrary;
...
if Operation = opRemove then
  ...
  if FChildLibraries.IndexOf( AComponent) <> -1 then
    (AComponent as TCryptographicLibrary).SetParentLibrary( nil)
  end;
end;
...
TCustomStreamCipherObj.Create( self)
end;
...
begin
if assigned( FPrstnt.OnGenerateKey) then
    result := FPrstnt.OnGenerateKey( FPrstnt.FLib, Seed)
  else
    result := nil
end;
...
begin
if assigned( FPrstnt.OnStart_Decrypt) then
    result := FPrstnt.OnStart_Decrypt( FPrstnt.FLib, Key, PlainText)
  else
    result := nil
end;
...
begin
if assigned( FPrstnt.OnStart_Encrypt) then
    result := FPrstnt.OnStart_Encrypt( FPrstnt.FLib, Key, CipherText)
  else
    result := nil
end;
