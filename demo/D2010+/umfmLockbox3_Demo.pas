...
if rgHashSource.ItemIndex <= 0 then
    ...
    LogFmt( 'SHA-384 of ansistring ''%s'' rendered in little-endien base64 is:', [a])
    end
  else
    begin
...
begin
(Sender as TAction).Enabled := (CurrentOperation = opHash) and
  (not pressedAbortHash)
end;


function TmfmLockbox3_Demo.ScribbleHashProgress(
  Sender: TObject; CountBytesProcessed: Int64): Boolean;
begin
result := True;
Application.ProcessMessages;
...
if Sig.isLoaded then
    ...
    LogFmt( 'libeay32.dll version is %d.%d.%d.%d', [Sig.MajorV, Sig.MinorV, Sig.ReleaseV, Sig.BuildV])
    end
  else
    begin
    Log( 'Failed to load library.');
    case Sig.Err of
      erLoadFailed, erException: Log( Sig.ErrMsg);
      erVersionTooLow:
        begin
        Log( 'Found libeay32.dll, but it was too lowly versioned. Update your OpenSSL binaries!');
        LogFmt( 'Required version is %s', [Sig.RequiredVersion])
        end;
      erSignatureAbsent: Log( 'Found libeay32.dll, but the API access seemed to be absent.');
      end
    end;
end;
...
begin
(Sender as TAction).Enabled := (not Sig.isLoaded) and (CurrentOperation = opIdle)
end;
...
function TmfmLockbox3_Demo.SigProgress(
  Sender: TObject; p1, p2: Integer): boolean;
begin
result := True;
...
if not dlgOpenPEM.Execute then exit;
try
...
try
  Store := TFileStream.Create( dlgOpenPEM.FileName, fmOpenRead);
  try
    Sig.LoadKeysFromStream( Store, [partPublic])
  finally
    Store.Free
    end;
...
if not dlgSavePEM.Execute then exit;
try
  doesPreExist := FileExists( dlgSavePEM.FileName);
  Store := TFileStream.Create( dlgSavePEM.FileName, FileStreamModes[ doesPreExist]);
  try
...
begin
(Sender as TAction).Enabled := partPrivate in Sig.HasParts
end;
...
try
  ...
  try
    if doesPreExist then
      Store.Size := 0;
    Sig.StoreKeysToStream( Store, [partPublic])
  finally
    Store.Free
    end;
...
begin
(Sender as TAction).Enabled := partPublic in Sig.HasParts
end;
...
Signature := TFileStream.Create( edtOpenSSLSignature.Text, FileStreamModes[ doesPreExist]);
if doesPreExist then
  Signature.Size := 0;
...
begin
(Sender as TAction).Enabled := (partPrivate in Sig.HasParts) and
                               (edtOpenSSLDocument.Text <> '') and
                               (edtOpenSSLSignature.Text <> '') and
                               (CurrentOperation = opIdle)
end;
...
begin
(Sender as TAction).Enabled := (partPublic in Sig.HasParts) and
                               (edtOpenSSLDocument.Text <> '') and
                               (edtOpenSSLSignature.Text <> '') and
                               (CurrentOperation = opIdle)
end;
